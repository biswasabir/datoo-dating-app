...
public class PaymentsActivity extends AppCompatActivity implements PaymentProductListSkuAdapter.ItemViewHolder.OnItemSelectedListener, PurchasesUpdatedListener {
    ...
    private String PAYMENT_WALL = "PAYMENTWALL";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        TabLayout.Tab paymentWall = tabLayout.newTab();
        paymentWall.setIcon(R.drawable.ic_credit_card);
        paymentWall.setText(R.string.payment_wall);

        if (Config.GOOGLE_PAY_ENABLED) tabLayout.addTab(googlePayment);
        if (Config.FLUTTER_WAVE_ENABLED) tabLayout.addTab(cardPayment);
        if (Config.PAYPAL_ENABLED) tabLayout.addTab(paypalPayment);
        if (Config.PAYMENTWALL_ENABLED) tabLayout.addTab(paymentWall);
        ...
        tabLayout.addOnTabSelectedListener(new TabLayout.OnTabSelectedListener() {
            ...
            public void onTabSelected(TabLayout.Tab tab) {
                ...
                } else if (tab == paypalPayment){
                    PaymentMethodSelected = PAYMENT_PAYPAL;
                    tab.getIcon().setColorFilter(QuickHelp.getColor(PaymentsActivity.this, R.color.light_blue_700), PorterDuff.Mode.SRC_ATOP);

                } else if (tab == paymentWall){
                    PaymentMethodSelected = PAYMENT_WALL;
                    tab.getIcon().setColorFilter(QuickHelp.getColor(PaymentsActivity.this, R.color.light_blue_700), PorterDuff.Mode.SRC_ATOP);
                }
    ...
    public void initPurchaseFlow(){
        ...
        if (PaymentMethodSelected.equals(PAYMENT_GOOGLE)){
        ...
        } else if (PaymentMethodSelected.equals(PAYMENT_WALL)){

            UnifiedRequest request = new UnifiedRequest();

            request.setPwProjectKey(Config.PAYMENT_WALL_PROJECT_KEY);
            request.setPwSecretKey(Config.PAYMENT_WALL_SECRET_KEY); //optional, if you add your secret key here, signature will generate from SDK but we highly recommend don't expose secret key to front end. You can use your server calculate it.
            request.setAmount(price);
            request.setCurrency(productModel.getPriceCurrencyCode());
            request.setItemName(productModel.getTitle());
            request.setItemId(productModel.getSku());
            request.setUserId(mCurrentUser.getObjectId());
            ...
            request.addCustomParam("own_order_id", String.valueOf(QuickHelp.generateUId()));
            request.addCustomParam("itemName", productModel.getDescription());
        ...
        if (requestCode == RaveConstants.RAVE_REQUEST_CODE && data != null) {
            ...
            switch (resultCode) {
                ...
                case ResponseCode.FAILED:
                    ...
                    QuickHelp.showNotification(PaymentsActivity.this, getString(R.string.failed_try_again_later), true);
                    break;
